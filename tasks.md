...
| VAD-01..06 | Verification adequacy: mutation adapters, scoring, property-based, differential, flaky quarantine, verifier diversity | 🚧 In Progress — **VAD-1, VAD-2, VAD-5 Qualified**: `pkg/adequacy`, 13 tests, A1–A9 invariants, six mutants caught. Adequacy signals with a population floor, policy thresholds, and the flaky quarantine — inconsistent evidence is inconclusive and never `passed`, consistent failure is not flakiness, and cross-revision disagreement is a regression rather than flake. VAD-3 (property-based generation), VAD-4 (differential testing) and VAD-6 (verifier diversity) need a test runner and routing that do not exist yet |
