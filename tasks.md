...
| EXE-A01c | Native Linux backend (namespaces + cgroup v2) | EXE-4, EXE-5, EXE-6 | ✅ Qualified | `pkg/sandbox/linux.go`, **stdlib `syscall`, no dependency**. ADR-0107 **Accepted**. Capability map is **probed at construction**, never declared: measured unprivileged on stock Ubuntu 24.04 (kernel 6.17), `network_deny` + `cpu`/`memory`/`process` limits + `process_confinement` are enforceable, `filesystem_scope` is not (`apparmor_restrict_unprivileged_userns=1` leaves a userns with `CapEff=0`). First backend to claim enforced process confinement, which forced the SBX-5 `process_escape` probe to exist |
...
| ID | Task | Status |
|---|---|---|
| AGT-B01 | Debug workflow | 🚧 In Progress — **DBG-1..DBG-6 Qualified**: `pkg/debug`, 8 tests, J1–J8 invariants, twenty-one mutants caught on the first pass. DBG-5's *"original"* is the requirement: a debugging session naturally ends at a narrowed-down case, and fixing until *that* passes is a different claim from the one the user made — the gap is where a symptom-fix lives — so completion compares the reproduction by value and refuses a green run against anything else. DBG-6's check is a flag separate from its result, because "we looked and found nothing" and "we did not look" produce the same empty list. DBG-3's *identified* alternative to removal is honoured, but instrumentation nobody decided about is neither. DBG-2's ranks are explicit integers, since list order is invisible in a report and two hypotheses at the same rank are not ranked. Remaining: the debug agent loop, instrumentation injection and the runtime evidence collectors |
...
| CTX-B01 | Deep graph and history | 🚧 In Progress — **§9.2 source provenance Qualified**: `pkg/taint` source catalogue, 6 tests, F1–F6 invariants, seventeen mutants caught. The gap this closes: `index.Cite` reads provenance from the index for repository files, but §9.2's other fourteen sources — Git history, PR comments, issue text, fetched pages, connectors, attachments — each reach the assembler by their own route ending in a hand-written `taint.Class`, and the zero Class is `UserTrusted`. A PR comment saying "ignore the previous instructions" arriving as though the user typed it is one forgotten field away. Sources are now enumerated and mapped once, with an unregistered source resolving to `Unknown`. **A test caught an error in my own reasoning**: I asserted review comments outrank commit messages (wider authorship) and the lattice disagreed — it ranks by what content is *used for*, and repository content is executed. Corrected the test, not the lattice. Remaining: the graph traversal and Git-history retrieval engines themselves, which are §9.2 *sources* rather than numbered requirements — CTX-1..10 are already covered by `pkg/index` |
| CTX-B02 | Fast Context subagent | 🚧 In Progress — a Fast Context subagent is a *delegation*, which INS-5 governs, and while checking that I found **INS-1..INS-8 had no owning task row at all** — only INS-5's handoff slice was covered, despite being PRD goal 14. Now **Qualified**: `pkg/manifest`, 8 tests, IM1–IM8 invariants, twenty mutants caught. INS-2 and INS-3 are one requirement twice: a summary saying "the user asked for tests first" becomes the only trace of a Rule that said something more specific, and the next turn follows the paraphrase — which looks exactly like working correctly. `Turn` holds the compaction summary as a field the package can see and never reads, the same shape as REV-1's implementer rationale. INS-5's carriage reports *changed* as well as added and removed, since same-id-different-content is what a mid-run edit produces. Remaining: the subagent's exploration loop and budget enforcement |
| WIKI-B01 | CodeWiki generation | 🚧 In Progress — **WIKI-1, WIKI-5..WIKI-10 Qualified**: `pkg/codewiki` generation layer, 7 tests, W9–W15 invariants, seventeen mutants caught. WIKI-10's *"but remains provider-policy compliant"* is the requirement, not the caveat — a dedicated documentation model is wired as a config special case near whatever picks models, which is where the policy check lives, so a deployment that forbids a provider ends up documented by it. There is no exemption field. WIKI-1 puts policy ahead of the user setting absolutely (INV-9's direction), and with both disabled the reported reason is policy, since blaming the user setting sends them to flip a switch that changes nothing. WIKI-5's refresh set is derived from WIKI-6's freshness rather than computed again, because two implementations of "which pages are affected" is how a page stays stale forever. WIKI-9 keeps a failed stage's already-written pages. Remaining: the generator itself, cluster planning (WIKI-11..15) and the wiki UI |
...
| BRS-B01 | Scripted browser verification | 🚧 In Progress — **BRS-1..BRS-5 Qualified**: `pkg/browser`, 8 tests, O1–O8 invariants, twenty-one mutants caught. BRS-2's brokering exists so the credential never enters the context the model can read — a session that logs in by having the agent type a password satisfies every word of "controlled" and none of the requirement — so material is refused and only broker references cross. BRS-1's profile is keyed on organization + workspace + profile rather than the caller's id, because §35 partitions storage both ways and "default" is what everything is called. BRS-5's quarantine is INV-13's sibling: `Release` returns the provenance class, always Web, and a refused release hands back no trusted class for a caller who ignores the error. Remaining: the browser driver, artifact storage and the scripted-verification runner |
| BRS-B02 | Full computer use on Linux | 🚧 In Progress — **BRS-6 Qualified** in `pkg/browser` (O7, O8). The profile list is deliberately *not* an allowlist: nil means no profiles rather than every profile, because "eligible for desktop automation, profile unspecified" is not a state a deployment should reach by omission — the opposite of how a nil allowlist reads elsewhere here, and what BRS-6's word *explicit* asks for. A stray empty entry in the list is refused too, since it would otherwise make the unnamed profile a wildcard. Remaining: the Linux desktop-automation backend itself (X11/Wayland input synthesis, screen capture) and its sandbox integration |
| EXE-B01 | Remote worker gateway | 🚧 In Progress — **WRK-1..WRK-8 Qualified**: `pkg/worker`, 9 tests, D1–D9 invariants, twenty-three mutants caught and one proved equivalent. WRK-2 is the trap: capabilities and labels arrive together in the obvious design, but a capability is what the machine *can do* (only the worker knows) and a policy label is what it *may be trusted with* (the worker is the last thing to ask) — so `Registration` has no label field and a compromised worker cannot advertise `pci-approved` into the work that gates. WRK-3 has the same shape: the organization is a server-supplied parameter, not a registration field, and the empty-claim-equals-empty-record case is refused separately because the mismatch check cannot see it. WRK-5's drain is kept out of health, since a planned upgrade and an outage are identical to a scheduler and opposite to an operator. Remaining: the registration transport, mTLS termination and the scheduler itself |
| EXE-B02 | Environment Blueprints/snapshots | 🚧 In Progress — **§23.20 validation, matching, differential rebuild and restore Qualified**: `pkg/blueprint`, 9 tests, X1–X9 invariants, twenty mutants caught. §35's immutable-reference invariant enforced at both levels — a base is a content digest or it is refused, because every readable way to name an image is mutable and a tag reproduces a different environment later while the snapshot id in the run record still matches. INV-13 applied to the feature that invites it: §23.20 surfaces setup commands as agent knowledge, a repository Blueprint's commands are authored by whoever can open a pull request, so `SetupKnowledge` returns the provenance class alongside the commands and `OriginRepository` is the zero value. Snapshots are matched rather than ranked; a pin restores the pinned snapshot rather than falling forward to the newest. Remaining: the builder itself, build-log storage and the worker-side restore |
| IDE-B01 | Agent Command Center | 🚧 In Progress — **§20.3 action admission and CRC-1's gate Qualified**: `pkg/commandcenter`, 7 tests, CC1–CC7 invariants, nineteen mutants caught on the first pass. CRC-1 says overlap is surfaced *before either run creates a PR* — the word *before* is the requirement, since a finding shown next to an enabled Create Pull Request button has been surfaced onto the screen and not surfaced in the sense that matters — so an unresolved overlap removes the action and the refusal names the overlap rather than the run state. Actions are derived from state rather than rendered wholesale and rejected later: a Resume button on a completed run is the product telling a user the run can be resumed. The organization is a parameter to `View`, not a `Filter` field, and is checked before the filter — a filter is something a user narrows and the boundary is not. Remaining: the Kanban surface itself and the remaining §20.3 filter dimensions (user, service identity, model, cost, date) |
| IDE-B02 | Local-to-remote handoff | 🚧 In Progress — **HND-1..HND-6 and INS-5 Qualified**: `pkg/handoff`, 9 tests, H1–H9 invariants, twenty mutants caught. HND-5's disclosure is *derived* from both environments rather than declared by the source — the same failure mode as a self-labelling worker, since a source that has not noticed the target lacks Docker will not disclose it. Only losses are reported, because listing gained tools turns a disclosure into a diff nobody reads. HND-3's second half is the one that gets lost: a reference from a trusted local session is still only a name, and the remote resolves it under its *own* authorization, so material is refused rather than carried. HND-2's unresolved questions distinguish nil (nobody collected them) from empty (there are none). INS-5's manifest difference is reported rather than refused — refusing would push users around the check — but a remote reporting no manifest at all is refused, since unknown-as-preserved reports nothing. Remaining: the transfer itself and the IDE approval surface |
...
| AGT-B02 | Session Insights | 🚧 In Progress — **§23.22 admission Qualified**: `pkg/insights`, 7 tests, Q1–Q7 invariants, twenty mutants caught. §23.22's *"visibly labeled as generated analysis"* is WIKI-3's rule with more force — a wiki page says what the code does and a reader can check it, while an insight is a claim about *people*, derived from a sample and phrased as advice, so it is acted on rather than verified. The label is a construction requirement, not a rendering choice, and there is deliberately no "verified" value. "Repeated corrections" carries a population floor on the runs that *showed* it, since two across fifty is a rarer coincidence rather than a stronger one. "Not checked" stays distinct from "absent", because telling a team to add a Rule that already exists destroys trust in the feature the first time it happens. Remaining: the classifiers themselves and the metric aggregation pipeline |
| QA-B01 | Autonomous benchmark | 🚧 In Progress — **admission layer Qualified**: `pkg/benchmark`, 16 tests, E1–E15 invariants, sixteen mutants caught. §37.2's fifteen bullets split into thirteen that are decision procedures and two that are not. Covered: fixed revisions, hidden-and-visible tests (a visible-only result is refused, not caveated), the explicit verification contract, the defined human-intervention allowance as a *per-task* claim distinct from the unattended subset, the unattended subset itself at zero corrective messages, recorded model routing, five-part cost normalization, the multi-trial floor for stochastic tasks, the failure taxonomy with an inadmissible zero class, Wilson confidence intervals, the versioned release, and release-level coverage of the three mandated task kinds (compaction fidelity with all six label categories, approval quality including seeded high-risk mutations, streaming overhead across every client surface). Reproducible environments are *declared* here and built by EXE. Not covered: running the tasks — QA-B01 completes when a harness executes a release and the §37.3 comparison has a baseline to compare against |
...
| VAD-01..06 | Verification adequacy: mutation adapters, scoring, property-based, differential, flaky quarantine, verifier diversity | 🚧 In Progress — **VAD-1, VAD-2, VAD-5 Qualified**: `pkg/adequacy`, 13 tests, A1–A9 invariants, six mutants caught. Adequacy signals with a population floor, policy thresholds, and the flaky quarantine — inconsistent evidence is inconclusive and never `passed`, consistent failure is not flakiness, and cross-revision disagreement is a regression rather than flake. **VAD-6 Qualified** in `pkg/review` — model family is declared configuration, not inferred from model-id strings, and an undeclared family fails closed. VAD-3 (property-based generation) and VAD-4 (differential testing) need a test runner |
| MRS-01..04 | Monorepo scale: sparse checkout, VFS spike, Shard Manager, 10M-file benchmark | 🚧 In Progress — **MRS-3 and MRS-4 Qualified**: `pkg/shard`, 9 tests, S1–S9 invariants, six mutants caught. Addressable non-overlapping layout, longest-prefix resolution, shard-scoped rebuild planning, per-shard permission filtering, and composition refusing to mix revisions. MRS-1 (sparse checkout) needs workspace materialization and MRS-2 (VFS) is a SHOULD-level spike; neither is library work |
...
| SEC-C01 | SSO/SCIM/service identities | 🚧 In Progress — **§17.4 session tokens Qualified**: `pkg/session`, 7 tests, N1–N7 invariants, six mutants caught. Fifteen-minute TTL ceiling policy may only tighten, rotating refresh tokens where a replay revokes the whole family rather than being rejected, and an allowlist of surfaces so an unrecognised one never holds credentials. OIDC/SCIM federation and the identity provider need a server | ⬜ Proposed |
| AUT-C01 | GitHub/GitLab events | 🚧 In Progress — **INT-1, INT-3, INT-4, INT-6..INT-9 Qualified**: `pkg/integration`, 9 tests, I1–I9 invariants, fifteen mutants caught. INT-8 is the load-bearing one and it forbids a *claim* rather than a behaviour: a Native tier is a line in a config file and the INT-9 qualification is a test run, so nothing stops the line being written first. A Native claim missing any of the seven areas is refused, not downgraded. INT-4's idempotency is not manufactured — a provider that sends no delivery identifier cannot claim it, because hashing the payload collapses genuinely distinct identical events and drops the second silently. Signature verification runs before the ledger records anything, so a forgery cannot burn the delivery id the real event will use; delivery ids are scoped per integration because two providers will both eventually emit "1". INT-2 belongs to SEC-C02's broker and INT-5's actual pagination/backoff needs the HTTP clients; provider-specific GitHub and GitLab adapters are the remaining work |
...
| REV-C01 | Full code-review product | 🚧 In Progress — **REV-1, REV-2, REV-6, REV-7 Qualified**: `pkg/review` product layer, 6 tests, R9–R14 invariants, seventeen mutants caught on the first pass. REV-1's isolation is from the implementer's *reasoning*, not its output — the diff must cross, the plan and notes and rationale must not, because a reviewer given the argument evaluates the argument and agrees with it, then reports no findings in a way that reads as corroboration. REV-7's *"without silently changing user policy"* is the load-bearing clause: dismissing a finding is a click and changing what a deployment enforces is not, so `ApplyFeedback` states `PolicyChanged: false` and policy moves only through an explicit call needing an actor and a reason. An accepted risk is suppressed but is *not* fed back as a false positive, since that teaches the reviewer to stop reporting real problems. Remaining: the review agent itself, the PR surface and REV-4's independent verification runner |
| CTX-C01 | Remote/shared indexing | 🚧 In Progress — **LCX-5 partial availability, LCX-8, LCX-9 and IAM-5 Qualified**: `pkg/sharedcontext`, 9 tests, L1–L9 invariants, twelve mutants caught. LCX-8 is written the way a requirement is written when it knows what argument it will hear — the PRD rejects "encrypted embeddings alone are searchable shared context" outright — so `Index.Encrypted` is recorded and consulted nowhere, and the encrypted and plain paths are asserted to agree in *both* directions. Three drops leave no trace for one reason: a visibly-filtered result is an existence oracle, so an unresolvable match, a cross-Space match and a denied match are each dropped silently and the total is post-filter. Authorization runs at query time across all ten `pkg/authz` dimensions, since an index-time filter answers the permission question as it stood when the crawler last ran. LCX-9 delegation cannot exceed its Space list, and an empty delegation conveys nothing rather than everything. Remaining: the Context Gateway service and the remote index transport itself |
| WIKI-C01 | Shared CodeWiki and annotations | 🚧 In Progress — **WIKI-11..WIKI-15, WIKI-21, WIKI-22 Qualified**: `pkg/codewiki` sharing layer, 7 tests, W16–W22 invariants, twenty-one mutants caught on the first pass. The PRD's rejected element 7 — public unauthenticated access to private Wiki content — is enforced structurally: the repository's visibility is a caller-supplied fact rather than a field of the share request, the same shape as WRK-3's organization. WIKI-14 keeps annotations in their own field rather than flagging statements, because a flag works until something appends to the wrong list and then WIKI-3's citation rule either rejects human prose or accepts uncited generated claims — and the second is the one that ships. WIKI-12's override is per path, so naming one page does not switch cluster planning off for the repository. WIKI-13's rename carries annotations, which are the one part of a wiki that cannot be regenerated. Remaining: the sharing transport, the MCP server (WIKI-23) and Ask integration (WIKI-16..20) |
...
| EXT-C01 | Registry and marketplace | 🚧 In Progress — **PLG-1..PLG-4 admission Qualified**: `pkg/plugin`, 8 tests, G1–G8 invariants, six mutants caught. Manifest permission declaration, a defined-but-empty allowlist that permits nothing, signature requirement distinguishing absent from unverified, revocation outranking a pin, and rollback that never offers a revoked version. The registry itself, SBOM retention (PLG-7) and the isolated hook runtime (PLG-5) need a server and a sandbox host | ⬜ Proposed |
| DEP-C01 | Team server Helm/Terraform | ⬜ Proposed — **assessed, and genuinely not admission-layer work.** §23 names no DEP-numbered requirements; this is Helm charts and Terraform modules, which are deployment artifacts rather than decision procedures. The one library-shaped piece would be a validator asserting that a generated deployment satisfies the platform invariants (credentials reachable only by the gateway, no public wiki for a private repository, worker attestation where policy requires it) — but each of those is already enforced at its own boundary by `pkg/gateway`, `pkg/codewiki` and `pkg/worker`, and a second check over generated YAML would restate them without strengthening anything. Recorded rather than left unexplained, because "no numbered requirements" is a claim worth being able to check |
| QA-C01 | Multi-tenant qualification | 🚧 In Progress — **R-TEN-01/04/06 conformance Qualified**: `pkg/tenancy/conformance`, T1–T5 obligations with two real subjects (`coordination.Overlaps`, `shard.Compose`) and two negative subjects proving the suite can fail. T1 is a control, so a surface that refuses everything cannot pass by being broken; T4 catches a correct refusal that names the tenant it refused on behalf of. R-TEN-02/03/05 need a repository layer, a cache and an authorization service | ⬜ Proposed |
